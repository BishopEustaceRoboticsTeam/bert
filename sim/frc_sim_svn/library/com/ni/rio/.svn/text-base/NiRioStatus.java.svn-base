/**
 * \file NiRioStatus.h \author Dave Madden <david.madden@ni.com> \date
 * 02/21/2006
 * <p/>
 ...
 * <p/>
 * The NiRioStatus class also defines the various status constants used by the
 * cRIO.
 * <p/>
 * @todo Why are we storing the status value in an IntByReference? We should be
 * able to use a simple int field.
 */
public class NiRioStatus {
	...
	/**
	 * Update the status code to the most serious of the existing status code or
	 * the new status code. If the resulting code is "fatal" throw
