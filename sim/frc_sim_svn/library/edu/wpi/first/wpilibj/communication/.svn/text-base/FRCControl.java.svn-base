...
public final class FRCControl {
	...
	/*
	 * static { getCommonControlDataFn.setTaskExecutor(taskExecutor);
	...
	public static void setErrorData(byte[] bytes, int length, int timeOut) {
		/*
		 * Pointer textPtr = new Pointer(bytes.length); textPtr.setBytes(0,
		 * bytes, 0, bytes.length); setErrorDataFn.call3(textPtr, length,
		 * timeOut);
        textPtr.free();
		 */
	...
	public static void setErrorData(Pointer textPtr, int length, int timeOut) {
		/*
		 * if (length > textPtr.getSize()) { throw new
		 * IllegalArgumentException(); } setErrorDataFn.call3(textPtr, length, timeOut);
		 */
		Simulator.fixme(FRCControl.class, Thread.currentThread(), "setErrorData stubbed");
	}
	...
	public static void setUserDsLcdData(byte[] bytes, int length, int timeOut) {
		/*
		 * Pointer textPtr = new Pointer(bytes.length); textPtr.setBytes(0,
		 * bytes, 0, bytes.length); setUserDsLcdDataFn.call3(textPtr, length,
		 * timeOut);
        textPtr.free();
		 */
