...
public final class UsageReporting {
	...
	public static void report(int resource, int instanceNumber, int context, String feature) {
		/*
		 * if (feature != null) { Pointer featureStringPointer =
		 * Pointer.createStringBuffer(feature);
		 * nUsageReporting_reportFn.call4(resource, instanceNumber, context,
		 * featureStringPointer); //featureStringPointer.free();//TODO check to
		 * see if this can get called or not } else {
		 * nUsageReporting_reportFn.call4(resource, instanceNumber, context,
		 * Pointer.NULL());
        }
		 */
