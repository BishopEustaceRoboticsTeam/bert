...
/**
 * @author bradmiller Handles raw data input from the FRC Kinect Server when
 * used with a Kinect device connected to the Driver Station. Each time a value
 * is requested the most recent value is returned. See Getting Started with
 * Microsoft Kinect for FRC and the Kinect for Windows SDK API reference for
 * more information
 * <p/>
 */
public class Kinect implements FrcBotSimComponent {
	...
	class header_block_t extends FRCControl.DynamicControlData {
...

		{
			...
			data = new header_t(
					null/*
					 * new
					 * Pointer(backingNativeMemory.address().toUWord().toPrimitive()
					 * + 2,
                    header_t.size)
					 */);
		}
	...
	/**
	 * Called by the other Kinect functions to check for the latest data This
	 * function will update the internal data structures with the most recent
	 * Kinect input
	 */
	void updateData() {
		int retVal;
	...
	/**
	 * Retrieve the GravityNormal vector from the Kinect device The w value
	 * returned from this method is always 0
	 * <p/>
	...
	/**
	 * Query the position of the detected skeleton The w value returned from
	 * this method is always 1
	 * <p/>
