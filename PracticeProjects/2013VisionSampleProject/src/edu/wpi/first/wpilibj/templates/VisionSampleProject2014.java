...
// the criteria for doing the particle filter operation
    ...
    double leftStickX;
    double xVal;
    Servo baseServo = new Servo(7);
    Servo midServo = new Servo(6);
    Servo topServo = new Servo(5);
    ...
    public void autonomous() {
        //System.out.println("Autonmous is running ?");
...
//ColorImage image;                           // next 2 lines read image from flash on cRIO
                ...
                BinaryImage thresholdImage =  image.thresholdHSV(87, 114, 135, 255, 35, 80); //image.thresholdRGB(0, 16, 44, 70, 8, 27);//  // keep only green objects
                ...
                System.out.println("number of partical detected: " + filteredImage.getNumberParticles());
                if(filteredImage.getNumberParticles() > 0)
                {
                    System.out.println("number of partical was greater than zero");
			...
			if(scoreCompare(scores[i], false))
			{
                            ...
                            System.out.println("the value is!!!!!!!!!!!!!!!!!!!!!!!!!!!!!: "+ report.center_mass_x);
                            if(report.center_mass_x <= 240){
                                xVal = report.center_mass_x - 480;
                                xVal = xVal * -1;
                                xVal = (xVal - 240) * (xVal - 240);
                                xVal = xVal * -0.00000868;
                                xVal = xVal + 0.5;
                                System.out.println("This is the value of xVal::::::: " + xVal);
                            }
                            else if (report.center_mass_x > 240){
                                xVal = report.center_mass_x - 480;
                                xVal = xVal * -1;                            
                                xVal = (xVal - 240) * (xVal - 240);
                                xVal = xVal * 0.00000868;
			...
			for (int i = 0; i < verticalTargetCount; i++)
			{
                ...
                //System.out.println("No particals detected");
                ...
                ex.printStackTrace();
                System.out.println(" axis cam except");
            } catch (NIVisionException ex) {
                ex.printStackTrace();
                System.out.println(" NI Vis cam except");
            }
        }
    }
    ...
    public void operatorControl() {
        while (isOperatorControl() && isEnabled()) {
            leftStickX = rc_.getLeftStickX() * -1;
            leftStickX = leftStickX /2;
            leftStickX = leftStickX + 0.5;
            midServo.set(leftStickX);
            topServo.set( .5 +(rc_.getLeftStickY() / 2));
        }
    }
